// Search Service for VS Code Extension
import { VSCodeContext, VSCodeSummary, SearchQuery, SearchResult } from '../types';
...
export class SearchService {
    ...
    private searchInContexts(contexts: VSCodeContext[], query: SearchQuery): SearchResult[] {
        const results: SearchResult[] = [];
    ...
    private searchInSummaries(summaries: VSCodeSummary[], query: SearchQuery): SearchResult[] {
        const results: SearchResult[] = [];
        const queryTerms = this.tokenize(query.query);
    ...
    private findHighlightedTerms(content: string, queryTerms: string[]): string[] {
        const contentTokens = this.tokenize(content.toLowerCase());
    ...
    async searchByWorkspace(workspaceName: string): Promise<SearchResult[]> {
        const contexts = await this.storageService.getContexts();
        const filteredContexts = contexts.filter(c => 
            c.metadata.workspaceName === workspaceName
        );
        
        return this.searchInContexts(filteredContexts, { query: '', sort: 'date' });
    }

    async searchByLanguage(language: string): Promise<SearchResult[]> {
        const contexts = await this.storageService.getContexts();
        const filteredContexts = contexts.filter(c => 
    ...
    async searchByDateRange(startDate: Date, endDate: Date): Promise<SearchResult[]> {
        const contexts = await this.storageService.getContexts();
        const filteredContexts = contexts.filter(c => 
