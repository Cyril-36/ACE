...
class BackgroundServiceWorker {
  ...
  private async getMetrics(): Promise<Record<string, string | number | boolean>> {
    ...
    return {
      ...
      'contextAggregator._pendingBatches': (contextAggregatorStatus._pendingBatches as number) || 0,
      'contextAggregator._totalPendingContexts': (contextAggregatorStatus._totalPendingContexts as number) || 0,
      'summarization.totalSummaries': ((summarizationStatus._metrics as any)?.totalSummarizations as number) || 0,
      'summarization.averageLength': ((summarizationStatus._metrics as any)?.avgProcessingTime as number) || 0,
      ...
      'privacy.totalDataPoints': (privacyDashboard.auditSummary.totalEvents as number) || 0,
      'privacy.retentionCompliance': (privacyDashboard.policy.dataRetention as number) > 0,
